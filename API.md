...
- **Returns**: 
  ...
  - `ESP_ERR_INVALID_STATE` if called before `init()`.
...
- **Returns**: 
  ...
  - `ESP_OK` on sucess, 
  - `ESP_ERR_TIMEOUT` on timeout,
  - `ESP_ERR_INVALID_STATE` if wifi is not started.
...
- **Returns**: 
  ...
  - `ESP_OK` on sucess, 
  - `ESP_ERR_TIMEOUT` on timeout,
  - `ESP_ERR_INVALID_STATE` if wifi is not started, stopped etc.
...
- **Parameters**:
  - `timeout_ms`: Max time to wait for disconnection.
- **Returns**: 

  - `ESP_OK` on sucess, 
  - `ESP_ERR_TIMEOUT` on timeout,
  - `ESP_ERR_INVALID_STATE` wifi is not connected.
...
Calls `esp_wifi_restore()` and clears all manager settings, including channel etc.

#### `bool is_credentials_valid() const`
- **Returns**: 
  - `true` if the current credentials are considered valid.
  - `false` if are not valid.
