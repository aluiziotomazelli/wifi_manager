...
extern "C" void app_main(void)
{
    ...
    // Step 4: Connect to the WiFi network (synchronous - blocks until connected or timeout)
    ESP_LOGI(TAG, "Connecting to WiFi network...");
    ret = wifi_mgr.connect(CONNECTION_TIMEOUT_MS);
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "Successfully connected to WiFi!");
        ESP_LOGI(TAG, "You now have an IP address and can communicate over the network");
    }
    else if (ret == ESP_ERR_TIMEOUT) {
        ESP_LOGW(TAG, "Connection timed out after %d ms", CONNECTION_TIMEOUT_MS);
    }
    else {
