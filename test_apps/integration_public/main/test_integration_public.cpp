...
TEST_CASE("Public: Factory Reset", "[wifi][lifecycle]")
{
    WiFiManager &wm = WiFiManager::get_instance();
    wm.deinit();
    wm.init();
    ...
    TEST_ASSERT_EQUAL(ESP_OK, wm.factory_reset());
    TEST_ASSERT_EQUAL(WiFiManager::State::INITIALIZED, wm.get_state());

    TEST_ASSERT_EQUAL(ESP_OK, wm.start(5000));
    TEST_ASSERT_EQUAL(WiFiManager::State::STARTED, wm.get_state());

    printf("Trying to connect after factory reset...\n");
    TEST_ASSERT_EQUAL(ESP_ERR_WIFI_PASSWORD, wm.connect(15000));
    TEST_ASSERT_EQUAL(WiFiManager::State::STARTED, wm.get_state());

    printf("Connecting to %s...\n", TEST_WIFI_SSID);
    wm.set_credentials(TEST_WIFI_SSID, TEST_WIFI_PASS);
    TEST_ASSERT_EQUAL(ESP_OK, wm.connect(5000));
    TEST_ASSERT_EQUAL(WiFiManager::State::CONNECTED_GOT_IP, wm.get_state());

    wm.deinit();
}

TEST_CASE("Public: Credentials Persistence", "[wifi][credentials]")
{
    WiFiManager &wm = WiFiManager::get_instance();
    wm.deinit();
    wm.init();
    ...
    printf("Connecting to %s without seting credentials...\n", TEST_WIFI_SSID);
    TEST_ASSERT_EQUAL(ESP_OK, wm.connect(5000));
    TEST_ASSERT_EQUAL(WiFiManager::State::CONNECTED_GOT_IP, wm.get_state());
